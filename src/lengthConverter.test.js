...
/* ESERCISE I
  ...
  - write tests for that function using the AAA framework
    - Arrange 
      - Add your "expected" variable
      - Add a "feet" variable to pass into the function
    - Act
      - Add a variable called "actual" that captures the value of the function call 
      - eg. `let actual = footToMeter(feet);`
    - Assert
      - Assert that the expected value matches the actual value
      - eg. `expect(actual).toBe(expected);`
  - Open the "Tests" tab on the right to make sure your test is passing. Fix any problems that are causing failing tests
